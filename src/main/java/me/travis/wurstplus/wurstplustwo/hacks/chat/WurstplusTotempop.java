        ...
        if (event.get_packet() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35) {
                ...
                if (WurstplusFriendUtil.isFriend(entity.getName())) {
                    WurstplusMessageUtil.send_client_message( red + "" + green + " TotemPop " + reset + grey + " > " + reset + bold + green + entity.getName() + reset + " popio " + bold + count + reset + " totems");
                } else {
                    WurstplusMessageUtil.send_client_message( red + "" + red+ " TotemPop " + reset + grey + " > " + reset + bold + red + entity.getName() + reset + " popio " + bold + count + reset + " totems");
                }
        ...
        for (EntityPlayer player : mc.world.playerEntities) {
            ...
            if (player.isDead || player.getHealth() <= 0) {
                ...
                if (WurstplusFriendUtil.isFriend(player.getName())) {
                    WurstplusMessageUtil.send_client_message( red + "" + green + " TotemPop " + reset + grey + " > " + reset + bold + green + player.getName() + reset + " ripio tras popiar " + bold + count + reset + " totems :/");
                } else {
                    WurstplusMessageUtil.send_client_message( red + "" + red + " TotemPop " + reset + grey + " > " + reset + bold + red + player.getName() + reset + " ripio tras popiar " + bold + count + reset + " totems lol");
                }
