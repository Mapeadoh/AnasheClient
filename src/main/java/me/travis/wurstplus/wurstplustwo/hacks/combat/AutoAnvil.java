...
{
    WurstplusSetting break_mode;
    WurstplusSetting target;
    WurstplusSetting tick_delay;
    WurstplusSetting blocks_per_tick;
    WurstplusSetting range;
    WurstplusSetting decrease;
    WurstplusSetting h_distance;
    WurstplusSetting min_h;
    WurstplusSetting fail_stop;
    WurstplusSetting anti_crystal;
    WurstplusSetting rotate;
    WurstplusSetting fast_anvil;
    ...
    public static boolean basicChecksEntity(final Entity pl) {
        return pl == AutoAnvil.mc.player || WurstplusFriendUtil.isFriend(pl.getName()) || pl.isDead;
    }
    ...
    public static EntityPlayer findLookingPlayer(final double rangeMax) {
        ...
        for (final EntityPlayer playerSin : AutoAnvil.mc.world.playerEntities) {
            if (!basicChecksEntity((Entity)playerSin) && AutoAnvil.mc.player.getDistance((Entity)playerSin) <= rangeMax) {
    ...
    private boolean placeBlock(final BlockPos pos, final int step) {
        ...
        if (AutoAnvil.mc.player.inventory.getStackInSlot(this.slot_mat[utilSlot]) != ItemStack.EMPTY) {
            ...
            if (this.pick_d && step == AutoAnvil.to_place.size() - 1) {
                final EnumFacing prova = WurstplusBlockInteractHelper.getPlaceableSide(new BlockPos(this.enemyCoords[0], this.enemyCoords[1], this.enemyCoords[2]));
                if (prova != null) {
