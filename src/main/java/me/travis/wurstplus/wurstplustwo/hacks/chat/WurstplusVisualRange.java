			...
			for (String name : peoplenew) {
				if (!people.contains(name)) {
					if (WurstplusFriendUtil.isFriend(name)) {
						WurstplusMessageUtil.send_client_message("OJOOOOOOO! un " + ChatFormatting.RESET + ChatFormatting.GREEN + name + ChatFormatting.RESET + " salvaje espaunio :0000000");
					} else {
						WurstplusMessageUtil.send_client_message("un " + ChatFormatting.RESET + ChatFormatting.RED + name + ChatFormatting.RESET + " aparecio");
					}
					people.add(name);
				}
			}
