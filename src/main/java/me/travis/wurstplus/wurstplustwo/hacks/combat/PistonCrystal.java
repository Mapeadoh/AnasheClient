...
public class PistonCrystal extends WurstplusHack
{
    ...
    WurstplusSetting breakType;
    WurstplusSetting placeMode;
    WurstplusSetting target;
    WurstplusSetting range;
    WurstplusSetting crystalDeltaBreak;
    WurstplusSetting blocksPerTick;
    WurstplusSetting supBlocksDelay;
    WurstplusSetting startDelay;
    WurstplusSetting pistonDelay;
    WurstplusSetting crystalDelay;
    WurstplusSetting midHitDelay;
    WurstplusSetting hitDelay;
    WurstplusSetting stuckDetector;
    WurstplusSetting maxYincr;
    WurstplusSetting blockPlayer;
    WurstplusSetting rotate;
    WurstplusSetting confirmBreak;
    WurstplusSetting confirmPlace;
    WurstplusSetting allowCheapMode;
    WurstplusSetting betterPlacement;
    WurstplusSetting bypassObsidian;
    ...
    public void update() {
        if (PistonCrystal.mc.player == null) {
            ...
            if (this.enemyCoordsDouble != null && this.aimTarget != null) {
                ...
                if (this.rotate.get_value(true) && (int) PistonCrystal.mc.player.posX != this.meCoordsInt[0] && (int) PistonCrystal.mc.player.posZ != this.meCoordsInt[2]) {
    ...
    private void breakCrystalPiston(final Entity crystal) {
        if (this.hitTryTick++ >= this.midHitDelay.get_value(1)) {
            ...
            if (this.rotate.get_value(true)) {
                lookAtPacket(crystal.posX, crystal.posY, crystal.posZ, (EntityPlayer) PistonCrystal.mc.player);
            }
                                ...
                                if ((distanceNowCrystal = PistonCrystal.mc.player.getDistance(crystalCordsAbs[0], crystalCordsAbs[1], crystalCordsAbs[2])) < addedStructure.distance && this.get_block(crystalCordsAbs[0], crystalCordsAbs[1], crystalCordsAbs[2]) instanceof BlockAir && this.get_block(crystalCordsAbs[0], crystalCordsAbs[1] + 1.0, crystalCordsAbs[2]) instanceof BlockAir && !someoneInCoords(crystalCordsAbs[0], crystalCordsAbs[2])) {
                                    ...
                                    if (!this.rotate.get_value(true) && this.betterPlacement.get_value(true)) {
                                        ...
                                        for (final int[] disp : this.disp_surblock) {
                                            ...
                                            if ((minFound = PistonCrystal.mc.player.getDistanceSqToCenter(blockPiston)) <= distancePist && (this.get_block(blockPiston.getX(), blockPiston.getY(), blockPiston.getZ()) instanceof BlockPistonBase || this.get_block(blockPiston.getX(), blockPiston.getY(), blockPiston.getZ()) instanceof BlockAir) && !someoneInCoords(crystalCordsAbs[0] + disp[0], crystalCordsAbs[2] + disp[2]) && this.get_block(blockPiston.getX() - crystalCordsRel[0], blockPiston.getY(), blockPiston.getZ() - crystalCordsRel[2]) instanceof BlockAir) {
                                    ...
                                    for (final int[] pos : this.disp_surblock) {
                                        ...
                                        if ((minNow = PistonCrystal.mc.player.getDistance(torchCoords[0], torchCoords[1], torchCoords[2])) < minFound && (!this.redstoneBlockMode || pos[0] == crystalCordsRel[0]) && !someoneInCoords(torchCoords[0], torchCoords[2]) && (this.get_block(torchCoords[0], torchCoords[1], torchCoords[2]) instanceof BlockRedstoneTorch || this.get_block(torchCoords[0], torchCoords[1], torchCoords[2]) instanceof BlockAir) && ((int)torchCoords[0] != (int)crystalCordsAbs[0] || (int)torchCoords[2] != (int)crystalCordsAbs[2])) {
    ...
    public static EntityPlayer findLookingPlayer(final double rangeMax) {
        ...
        for (final EntityPlayer playerSin : PistonCrystal.mc.world.playerEntities) {
            if (!basicChecksEntity((Entity)playerSin) && PistonCrystal.mc.player.getDistance((Entity)playerSin) <= rangeMax) {
                listPlayer.add(playerSin);
            }
        }
        EntityPlayer target = null;
        final Vec3d positionEyes = PistonCrystal.mc.player.getPositionEyes(PistonCrystal.mc.getRenderPartialTicks());
        final Vec3d rotationEyes = PistonCrystal.mc.player.getLook(PistonCrystal.mc.getRenderPartialTicks());
    ...
    public static boolean basicChecksEntity(final Entity pl) {
        return pl == PistonCrystal.mc.player || WurstplusFriendUtil.isFriend(pl.getName()) || pl.isDead;
    }
    ...
    public static EntityPlayer findClosestTarget(final double rangeMax, final EntityPlayer aimTarget) {
        ...
        for (final EntityPlayer entityPlayer : playerList) {
            if (!basicChecksEntity((Entity)entityPlayer)) {
                if (aimTarget == null && PistonCrystal.mc.player.getDistance((Entity)entityPlayer) <= rangeMax) {
                    closestTarget_test = entityPlayer;
                }
                else {
                    if (aimTarget == null || PistonCrystal.mc.player.getDistance((Entity)entityPlayer) > rangeMax || PistonCrystal.mc.player.getDistance((Entity)entityPlayer) >= PistonCrystal.mc.player.getDistance((Entity)aimTarget)) {
                        continue;
                    }
                    closestTarget_test = entityPlayer;
                }
    ...
    public static void setYawAndPitch(final float yaw1, final float pitch1) {
        PistonCrystal.yaw = yaw1;
        PistonCrystal.pitch = pitch1;
